// wall-probe.ts — §5d INSTRUMENTO DE PARED (cero ajuste visual).
//
// Tras bandera ?debug=walls (modo 1: R=pendiente G=rockK B=croma(albedo)) o
// ?debug=walls2 (modo 2: R=pendiente G=b−r(final, centrado 0,5) B=luma(final)).
// Carga diferida desde viewer.ts: producción ni lo importa ni lo llama.
//
// Diseño:
...
// - El fragment de la pieza escribe las g* DESPUÉS de dithering (se salta
//   tonemapping + colorspace: lo que lee readPixels es el valor escrito).
...
//   rockKHist (10 cubos), cromaAbyS (walls), brFbyS + lumaFbyS (walls2).
...
export function mountWallProbe(wire: Wire): {
  ...
  const { renderer, scene, camera, sky, lineGroup, cloudsGroup, beamsGroup, labelLayer, uWallProbe } = wire;
  const R_MARK = 0.04;
  ...
  function renderProbe(mode: 1 | 2): Uint8Array | null {
  ...
  return {
    ...
    readHist(mode: 1 | 2): WallHist | null {
      ...
      const cromaAcc: { sum: number; n: number }[] = Array.from({ length: 18 }, () => ({ sum: 0, n: 0 }));
      ...
      for (let i = 0; i < w * h; i++) {
        ...
        if (mode === 1) {
          ...
          (cromaAcc[bin] as { sum: number; n: number }).sum += bb;
          ((cromaAcc[bin] as { sum: number; n: number }).n)++;
        } else {
