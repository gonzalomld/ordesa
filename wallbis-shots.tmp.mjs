// wallbis-shots.tmp.mjs — §5d-bis: solo on/off + G104 (los histogramas ya salieron).
...
const shot = (page, path) => page.screenshot({ path, timeout: 120000 });
...
async function shadow104(s, name) {
  ...
  await shot(page, `/tmp/wallbis-shadow-${name}.png`);
...
const zens = {};
zens.s029 = await shadow104('0.29', 's029');
zens.s080 = await shadow104('0.80', 's080');
zens.actoV = await shadow104('0.97', 'actoV');
await browser.close();
