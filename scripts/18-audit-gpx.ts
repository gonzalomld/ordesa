...
//   src/generated/gaps.ts    — gap intervals in along-track d (m) for the
//                              ?debug=gaps overlay (lazy chunk, never prod)
...
// --- front overlay intervals: along-track d (m) spanned by each gap ---
// The resampled route point with distance d is invented iff d falls inside
// one of these raw intervals (05-build-route interpolates across the gap).
const ranges = gaps.map(
  (g): [number, number] => [
    Math.round((cum[g.index] as number) * 10) / 10,
    Math.round(((cum[g.index] as number) + g.distM) * 10) / 10,
  ],
);
...
writeFileSync(
  ...
  `// GENERATED by scripts/18-audit-gpx.ts — do not edit by hand.\n` +
    `// Gap intervals as along-track distances (m): resampled route points\n` +
    `// whose d falls inside a range are straight-line inventions of the raw\n` +
    `// GPX (pre-resample), painted magenta by ?debug=gaps.\n` +
    `export const GAP_RANGES: Array<readonly [number, number]> = ${JSON.stringify(ranges)};\n` +
    `export const GAP_META = ${JSON.stringify({ count: gaps.length, interpM: out.interpM, totalM: out.totalM, generatedAt: out.generatedAt })};\n`,
);
console.log(`saved: ${OUT_GEN} (${ranges.length} ranges)`);
